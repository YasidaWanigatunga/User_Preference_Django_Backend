"""
URL configuration for config project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.1/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from django.contrib.auth import views as auth_views
from preferences.views import register, user_login, account_settings, user_logout, notification_settings, theme_settings


urlpatterns = [
    path('admin/', admin.site.urls),
    path('register/', register, name='register'),
    path('login/', user_login, name='login'),
    path('preferences/', account_settings, name='preferences'),
    path('logout/', user_logout, name='logout'),
    path('notification-settings/', notification_settings, name='notification_settings'),
    path('theme-settings/', theme_settings, name='theme_settings'),
    path('', register, name='home'),
    path('accounts/login/', auth_views.LoginView.as_view(), name='default_login'),
]

